...
const DIV_WALLET_WRAPPER_HEADER = "wallet-wrapper-header-account";
const DIV_WALLET_WRAPPER_HEADER_CLOSE = "wallet-wrapper-header-close";

export class WalletWrapperMobile {
  driver: WebDriver;
  ...
  async isAccInfoDisplayed(accName: string) {
    const walletStatusAcc =
      buildDataTestIdXpath(DIV_WALLET_WRAPPER_HEADER) +
  ...
  async openWalletConnectionInfo() {
    ...
    if (!isDepositDisplayed) {
      await clickElement(this.driver, walletStatus);
    }
  }

  async waitWalletOpened() {
    const depositButton = buildXpathByElementText("button", "Deposit");
    const acc_header = buildDataTestIdXpath(DIV_WALLET_WRAPPER_HEADER);
    try {
      await waitForElementVisible(this.driver, depositButton, 5000);
      await waitForElementVisible(this.driver, acc_header, 5000);
    } catch (error) {
      // Element not found or other error occurred
    }
  }

  async waitWalletClosed() {
    const depositButton = buildXpathByElementText("button", "Deposit");
    const acc_header = buildDataTestIdXpath(DIV_WALLET_WRAPPER_HEADER);
    try {
      await waitForElementToDissapear(this.driver, depositButton);
      await waitForElementToDissapear(this.driver, acc_header);
    } catch (error) {
      // Element not found or other error occurred
    }
  }

  async closeWalletConnectionInfo() {
    const walletStatusClose = buildDataTestIdXpath(
      DIV_WALLET_WRAPPER_HEADER_CLOSE,
    );
    const depositButton = buildXpathByElementText("button", "Deposit");
    const isDepositDisplayed = await isDisplayed(this.driver, depositButton);
    if (isDepositDisplayed) {
      await clickElement(this.driver, walletStatusClose);
    }
  }

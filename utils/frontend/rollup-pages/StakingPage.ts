...
export class StakingPage {
  ...
  async waitForCollatorsListVisible() {
    const collatorListLocator = buildDataTestIdXpath("active-collators-list");
  ...
  async waitForCollatorsVisible() {
    ...
    await waitForElementVisible(this.driver, collatorListLocator, 60000);
  }
  ...
  async getCollatorInfo(collatorType: string) {
    ...
    const minBondXpath =
      collatorListXpath + buildDataTestIdXpath("min-bond-value");
  ...
  async getCollatorsStakes(collatorsType: string) {
    ...
    for (let i = 0; i < stakes.length; i++) {
      await scrollElementIntoView(this.driver, stakes[i]);

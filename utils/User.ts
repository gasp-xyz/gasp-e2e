...
export class User {
  ...
  constructor(keyring: Keyring, name = "", json: any = undefined) {
    ...
    if (!name) {
      // name = "//testUser_" + uuid();
      name = "0x" + randomBytes(32).toString("hex");
      autoGenerated = true;
    }
    this.name = name;
    ...
    if (json) {
      ...
      if (keyring.type !== "ethereum") {
        testLog.getLog().info("Ok Ok , old  keyring ?" + keyring.type);
        this.keyRingPair = keyring.createFromUri(name);
      } else if (!autoGenerated) {
        //@ts-ignore
        const address = new ethers.Wallet(this.name).address;
        this.keyRingPair = keyring.createFromUri(name);
        testLog
          .getLog()
          .info(
            "User with name -> will fail " + this.name + " address: " + address,
          );
      } else {
        this.keyRingPair = keyring.addFromUri(name);
      }
      const ethAddress = this.keyRingPair.address;
      testLog.getLog().debug(this.keyRingPair.address);
      Object.defineProperty(this.keyRingPair, "address", {
        get: function () {
          return ethAddress.toLowerCase();
        },
      });
      testLog.getLog().debug(this.keyRingPair.address);
      this.ethAddress = this.keyRingPair.address;
    }
    this.assets = [];
    if (autoGenerated) {
      testLog

...
describe("Proof of stake tests", () => {
  ...
  describe("Claim rewards scenarios", () => {
    ...
    it("Rewards are divided in n-sessions", async () => {
      ...
      await waitForRewards(testUser, liquidityAssetId, 80, GASP_ASSET_ID);
    ...
    it("Two users activated, one in one exec, other in two - check balances", async () => {
      ...
      await waitForRewards(oneUser, liquidityAssetId, 40, GASP_ASSET_ID);
      ...
      await waitForRewards(otherUser, liquidityAssetId, 40, GASP_ASSET_ID);
      const balanceOne = await getUserBalanceOfToken(GASP_ASSET_ID, oneUser);
      const balanceOther = await getUserBalanceOfToken(
        GASP_ASSET_ID,
        otherUser,
      );
      ...
      expect(
        (await getUserBalanceOfToken(GASP_ASSET_ID, oneUser)).free,
      ).bnEqual(balanceOne.free.add(rewardsOne2));
      expect(
        (await getUserBalanceOfToken(GASP_ASSET_ID, otherUser)).free,

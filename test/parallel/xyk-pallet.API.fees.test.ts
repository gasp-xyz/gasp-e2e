...
test('xyk-pallet - MGA tokens are substracted as fee : CreatePool', async () => {
	...
	const [thirdCurrency, fourthCurrency] = await Assets.setupUserWithCurrencies(testUser1, [defaultCurrecyValue,defaultCurrecyValue.add(new BN(1))], sudo);
	await createPool(testUser1.keyRingPair, thirdCurrency, new BN(1000),fourthCurrency,new BN(100));
...
test('xyk-pallet - MGA tokens are substracted as fee : MintLiquidity', async () => {
	...
	await mintLiquidity(testUser1.keyRingPair, firstCurrency, secondCurrency, new BN(1000))
	await testUser1.refreshAmounts(AssetWallet.AFTER);
	const mgaUserToken = testUser1.getAsset(MGA_ASSET_ID)!;
	const diff = mgaUserToken.amountBefore.toNumber() - mgaUserToken.amountAfter.toNumber();
	expect(diff).toBeGreaterThan(0);
...
test('xyk-pallet - MGA tokens are substracted as fee : BurnLiquidity', async () => {
	...
	await burnLiquidity(testUser1.keyRingPair, firstCurrency, secondCurrency, new BN(1000))
	await testUser1.refreshAmounts(AssetWallet.AFTER);
	const mgaUserToken = testUser1.getAsset(MGA_ASSET_ID)!;
	const diff = mgaUserToken.amountBefore.toNumber() - mgaUserToken.amountAfter.toNumber();
	expect(diff).toBeGreaterThan(0);
...
test('xyk-pallet - MGA tokens are substracted as fee : Transfer', async () => {
	...
	await transferAsset(testUser1.keyRingPair, firstCurrency,testUser2.keyRingPair.address, new BN(1000))
	await testUser1.refreshAmounts(AssetWallet.AFTER);
	const mgaUserToken = testUser1.getAsset(MGA_ASSET_ID)!;
	const diff = mgaUserToken.amountBefore.toNumber() - mgaUserToken.amountAfter.toNumber();
	expect(diff).toBeGreaterThan(0);
...
test('xyk-pallet - MGA tokens are substracted as fee : TransferAll', async () => {
	...
	await transferAll(testUser2.keyRingPair, firstCurrency,testUser1.keyRingPair.address)
...
test('xyk-pallet - MGA tokens are not substracted as fee : SellAsset', async () => {
	...
	const diff = mgaUserToken.amountBefore.toNumber() - mgaUserToken.amountAfter.toNumber();
	expect(diff).toBe(0);
	expect(testUser1.getAsset(firstCurrency)!.amountBefore.toNumber()).toBeLessThan(testUser1.getAsset(MGA_ASSET_ID)!.amountAfter.toNumber());
...
test('xyk-pallet - MGA tokens are not substracted as fee : BuyAsset', async () => {
	...
	const diff = mgaUserToken.amountBefore.toNumber() - mgaUserToken.amountAfter.toNumber();
	expect(diff).toBe(0);
	expect(testUser1.getAsset(firstCurrency)!.amountBefore.toNumber()).toBeGreaterThan(testUser1.getAsset(firstCurrency)!.amountAfter.toNumber());
	pallet.validateWalletIncreased(MGA_ASSET_ID, new BN(0));
});

...
import { getBalanceOfPool, getLiquidityAssetId, burnLiquidity, getBalanceOfAsset, calculate_buy_price_local} from '../../utils/tx'
...
describe('xyk-pallet - Burn liquidity tests: when burning liquidity you can', () => {
	...
	test('Burning liquidities provides Burn and settle', async () => {
		...
		await burnLiquidity(testUser1.keyRingPair, firstCurrency, secondCurrency, new BN(defaultCurrecyValue).div(new BN(4)))
		.then(
			(result) => {
				const eventResponse = getEventResultFromTxWait(result, ["xyk", "LiquidityBurned", testUser1.keyRingPair.address]);
				expect(eventResponse.state).toEqual(ExtrinsicResult.ExtrinsicSuccess);
	...
	test('Burning liquidities generates a Liquidity burned event', async () => {
		...
		await burnLiquidity(testUser1.keyRingPair, firstCurrency, secondCurrency, burnAmount)
		.then(
			(result) => {
				eventResponse = getEventResultFromTxWait(result, ["xyk", "LiquidityBurned", testUser1.keyRingPair.address]);
				expect(eventResponse.state).toEqual(ExtrinsicResult.ExtrinsicSuccess);
			}
		);
		...
		validateMintedLiquidityEvent(eventResponse, testUser1.keyRingPair.address, firstCurrency, firstCurrencyAmount, secondCurrency, secondCurrencyAmount, liquidityAssetId, burnAmount);	

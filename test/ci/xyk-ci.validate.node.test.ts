...
test('xyk-CI - Node is up and running', async () => {
	...
	let health = await ( api.rpc as any).system.health();
	testLog.getLog().info('Node health : ' + health.toString());
	expect(health.toString()).not.toBeUndefined();
		
	let version = await ( api.rpc as any).system.version();
	...
	const [headNo1, headNo0] = [JSON.parse(heads[1].toString()).number , JSON.parse(heads[0].toString()).number];
